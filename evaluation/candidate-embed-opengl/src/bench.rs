...
//! is [`crate::screen::Screen::apply`] and [`crate::gl::build_scene`], the same
//! functions the live window calls.

use rmpv::Value;

use crate::ext_ui::{ExtUi, Overlay};
...
fn run_frame(
    ...
    // No external surfaces in the benchmark: it measures the grid path, and an
    // idle overlay is what a session without a cmdline or popup actually has.
    let build = recorder.span();
    gl::build_scene(verts, screen, atlas, "", &ExtUi::new(), &Overlay::default());
...
pub fn run(params: &BenchParams) -> perf::Report {
    ...
    let mut screen = Screen::new(params.cols, params.rows);
    ...
    screen.apply(&prologue);
    for _ in 0..params.warmup {
        let events = workload.next_frame();
        run_frame(&mut recorder, &mut screen, &mut atlas, &mut verts, &events);
