...
impl ExtUi {
    ...
    /// defined the group.
    pub fn colors(&self, surface: &impl Surface, hl: HlRef) -> (u32, u32) {
        match hl {
            HlRef::Attr(id) => surface.hl_colors(id),
            HlRef::Ui(group) => match self.ui_groups.get(group.group()) {
                Some(&id) => surface.hl_colors(id),
                None => (
                    surface.default_fg(),
                    blend(surface.default_bg(), surface.default_fg(), group.fallback_mix()),
                ),
            },
        }
    }
...
mod tests {
    ...
    fn grid_with_hl(id: u64, fg: u32, bg: u32) -> crate::screen::Screen {
        let mut grid = crate::screen::Screen::new(4, 1);
        grid.apply(&[(
    ...
    fn unmapped_ui_groups_fall_back_to_a_shade_of_the_default_background() {
        let grid = crate::screen::Screen::new(4, 1);
        let ui = ExtUi::new();
        let (fg, pmenu_bg) = ui.colors(&grid, HlRef::Ui(UiHl::Pmenu));
        let (_, sel_bg) = ui.colors(&grid, HlRef::Ui(UiHl::PmenuSel));
        assert_eq!(fg, Surface::default_fg(&grid));
        assert_ne!(pmenu_bg, Surface::default_bg(&grid), "the box must be distinguishable");
        assert_ne!(
            sel_bg, pmenu_bg,
    ...
    fn attribute_spans_go_through_the_grid_highlight_table() {
        ...
        assert_eq!(
            ...
            (Surface::default_fg(&grid), Surface::default_bg(&grid))
        );
    }
